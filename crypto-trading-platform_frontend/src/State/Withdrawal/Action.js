...
export const getAllWithdrawalRequest = jwt => async dispatch => {
  ...
  export const addPaymentDetails = ({jwt,paymentDetailsRequest}) => async dispatch => {
    dispatch({ type: ADD_PAYMENT_DETAILS_REQUEST });
    try {
      const response = await api.post('/api/payment-details',paymentDetailsRequest, {
        headers: { Authorization: `Bearer ${jwt}` }
      });
      console.log(paymentDetailsRequest);
      dispatch({
